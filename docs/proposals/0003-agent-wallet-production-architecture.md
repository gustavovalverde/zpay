...
### D-3: PCZT (ZIP-374) is the signed artifact, wrapped in a chain-neutral envelope on the wire
...
**How to apply.** The wire field is `signed_payload`. It is an object: `{ format: "pczt-v2-extractable" | <future>, bytes: <base64>, tx_id, fee, expires_at, metadata: { chain_specific: {...} } }`. The Rust type is `SignedPayload`. PCZT is one value of `format`. The wallet returns the PCZT in extractor-ready state (all proofs and signatures present, all inputs signed with SIGHASH_ALL only). zpay's role is Extractor plus broadcast; it refuses any PCZT that is not extractor-ready.
